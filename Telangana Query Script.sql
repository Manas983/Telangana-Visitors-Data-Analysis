...
#Adding new columns
ALTER TABLE population_2011
ADD COLUMN Estimated_2019 integer, -- Adjust data type as per your requirement
ADD COLUMN Estimated_2025 integer;
...
#update new column
UPDATE population_2011 
SET Estimated_2019 = ROUND(Population * POWER(1.11, 8)), # Creating new column Estimated population for 2019
    Estimated_2025 = ROUND(Population * POWER(1.11, 14));  # Creating new column Estimated population for 2025
...
SELECT 
  ...
  (SUM(domestic_visitors.visitors) + SUM(foreign_visitors.visitors)) / population_2011.Estimated_2019 AS footfall_ratio
FROM domestic_visitors
JOIN population_2011 ON population_2011.District = domestic_visitors.district
JOIN foreign_visitors ON domestic_visitors.district = foreign_visitors.district AND domestic_visitors.year = foreign_visitors.year
...
SELECT 
  ...
  (SUM(domestic_visitors.visitors) + SUM(foreign_visitors.visitors)) / population_2011.Estimated_2019 AS footfall_ratio
FROM domestic_visitors
JOIN population_2011 ON domestic_visitors.district = population_2011.District
JOIN foreign_visitors ON domestic_visitors.district = foreign_visitors.district AND domestic_visitors.year = foreign_visitors.year
WHERE domestic_visitors.year = 2019
GROUP BY domestic_visitors.district, population_2011.Estimated_2019
